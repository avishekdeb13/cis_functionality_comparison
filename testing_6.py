...
def extract_components(soup):
    comps = {
        "forms": [], "buttons": [],
        "inputs": [], "tables": [], "dropdowns": []
    }

    # Forms
    for i, form in enumerate(soup.find_all("form"), 1):
        method = form.get("method", "GET").upper()
        action = form.get("action", "")[:30]
        comps["forms"].append("Form" + str(i) + "[" + method + "]" + action)

    # Buttons
    seen = set()
    for btn in soup.find_all(["button", "input"]):
        t = btn.get("type", "").lower()
        if t in ["submit", "button", "reset"] or btn.name == "button":
    ...
    for inp in soup.find_all("input"):
        ...
        if t in ["hidden", "submit", "button", "reset"]:
            continue
        name = (inp.get("placeholder", "") or
                inp.get("name", "") or
                inp.get("id", ""))
        k = t + ":" + name
        if k not in seen2:
            comps["inputs"].append(k[:35])
            seen2.add(k)
...
def write_excel(rows, results, cfg):
    ...
    s2_cols = ["#", "Main Menu", "Sub Menu", "URL", "Result", "Issue",
               "Load(s)", "Page Title", "Forms", "Buttons",
               "Input Fields", "Tables", "Dropdowns", "Screenshot"]
    s2_wids = [4, 20, 25, 48, 11, 40, 7, 32, 22, 28, 28, 28, 28, 32]
    ...
    for ri, r in enumerate(results, 2):
        ...
        vals = [
            ri-1, r["main"], r["sub"],
            r["url"] or r.get("href",""),   # always show something in URL col
            st, r["issue"], r["load"], r["title"],
            r["forms"], r["buttons"], r["inputs"],
            r["tables"], r["dropdowns"], r["screenshot"]
        ]
        for ci, val in enumerate(vals, 1):
    ...
    ws2.freeze_panes = "A2"
    ws2.auto_filter.ref = "A1:N" + str(len(results)+1)
