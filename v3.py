...
def get_user_config():
    print("=" * 62)
    print("  DRT Portal Scraper & Tester  —  Universal Edition")
    print("=" * 62)
    print("\nEnter site details (press ENTER to use DRT default):\n")

    default_url  = "https://cis.drt.gov.in/drtlive/index.php"
    default_user = "filingdrt1"
    default_pass = "Sodrt1@6060"

    url  = input(f"  Site URL      [{default_url}]: ").strip() or default_url
    user = input(f"  Username      [{default_user}]: ").strip() or default_user
    pwd  = input(f"  Password      [{'*'*len(default_pass)}]: ").strip() or default_pass

    # Derive safe filename prefix from domain
