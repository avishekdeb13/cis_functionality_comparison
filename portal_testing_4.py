...
USER_LOCS = [
    ...
    (By.NAME,  "txtUsername"),
    (By.ID,    "txtUsername"),
    # Angular / React portals use formControlName
...
def login(driver, pcfg, captcha_wait):
    ...
    # Wait up to 8s for inputs to appear (handles Angular lazy rendering)
    try:
    ...
    # If already on dashboard (session still alive), skip fill
    ...
    def fill(label, val, locs):
        for loc in locs:
            try:
                ...
                pass
        print(f"  ! {label} not found — fill manually in browser")
        return False
...
def extract_page_content(html):
    ...
    # ── Angular / React fields outside <form> tags ────────────────────────────
    # Catch inputs that Angular renders without wrapping <form>
    seen_labels = {f["label"].lower() for f in result["fields"]}
    ...
    for inp in soup.find_all("input"):
        ...
        # Try Angular-specific attributes first
        fc_name  = (inp.get("formcontrolname","") or
                    inp.get("ng-reflect-name","") or
                    inp.get("ng-model","") or
                    inp.get("ngmodel",""))
        lbl      = find_label(inp)
        ph       = inp.get("placeholder","").replace("*","").strip()
        nm       = inp.get("name","") or inp.get("id","") or fc_name
        display  = lbl or ph or fc_name or nm
        if not display:
            continue
        if display.lower() in seen_labels:
            continue
        if display.lower() in ("search","search for...",""):
            continue

        dtype = detect_datatype(inp)
        mand  = (is_mandatory(inp) or
                 inp.has_attr("required") or
                 "*" in inp.get("placeholder",""))
        result["fields"].append({
            "form":      0,
            "form_name": "angular-form",
            "label":     display[:60],
            "datatype":  dtype,
            "mandatory": "MANDATORY" if mand else "OPTIONAL",
            "options":   "",
    ...
    for sel in soup.find_all("select"):
        lbl      = find_label(sel)
        fc_name  = (sel.get("formcontrolname","") or
                    sel.get("ng-reflect-name","") or "")
        nm       = sel.get("name","") or sel.get("id","") or fc_name
        display  = lbl or fc_name or nm
        if not display or display.lower() in seen_labels:
            continue
        opts = [o.get_text(strip=True) for o in sel.find_all("option")
                if o.get_text(strip=True)]
        mand = is_mandatory(sel) or sel.has_attr("required")
        result["fields"].append({
            "form":      0,
            "form_name": "angular-form",
            "label":     display[:60],
...
# ─────────────────────────────────────────────────────────────────────────────
def wait_for_content(driver, timeout=8):
    """
    Wait for JS frameworks (Angular/React) to finish rendering.
    Tries 3 signals:
      1. document.readyState == complete
      2. Angular: window.getAllAngularTestabilities() all stable
      3. At least 1 visible input/table/form exists OR 3s passed
    """
    # Signal 1: DOM ready
    try:
    ...
    # Signal 2: Angular stability (only if Angular is present)
    try:
        is_angular = driver.execute_script(
    ...
    # Signal 3: Wait until inputs or tables appear (max 5s extra)
    try:
        WebDriverWait(driver, 5).until(
            lambda d: (
                len(d.find_elements(By.CSS_SELECTOR,
                    "input:not([type=hidden]), select, textarea, table")) > 0
            ))
    except TimeoutException:
        pass  # page may genuinely have no inputs — that is fine

    time.sleep(0.8)  # final settle


def visit_page(driver, url, label, timeout, portal_label,
               do_interact=False):
    """
    Navigate directly to URL. Extract content. Never use back().
    Handles both server-rendered (old portal) and JS-rendered (Angular/React) pages.
    do_interact=True: select first dropdown option (TEST only, no form submit).
    """
    ...
    try:
        ...
        # For Angular portals: also try to extract from shadow DOM / ng-content
        # by scrolling so lazy components render
        try:
        ...
        if do_interact:
            for sel_el in driver.find_elements(By.CSS_SELECTOR, "select")[:5]:
                try:
                    ...
                    pass
            # After dropdown changes Angular may re-render — wait again
            try:
                wait_for_content(driver, 4)
            except Exception:
                pass

        html = driver.page_source
        res["content"] = extract_page_content(html)

        nf = len(res["content"]["fields"])
        nt = len(res["content"]["tables"])
        nb = len(res["content"]["buttons"])
        print(f"    [{portal_label}] {label[:48]:<48}  "
              f"fields={nf}  tables={nt}  buttons={nb}  {res['status']}")

    except TimeoutException:
        res["status"] = "TIMEOUT"
        try:
            # Even on timeout, try to extract what rendered
